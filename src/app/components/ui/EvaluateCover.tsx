...
export default function EvaluateCover({ isExpanded = false, isHovered = false, isActive = true, onCycleComplete }: EvaluateCoverProps) {
  ...
  useEffect(() => {
    ...
    const tl = gsap.timeline({
      ...
      onComplete: () => {
        if (isHoveredRef.current) {
          // Hovering: loop — reset front, flip back to front, restart
          NODES.forEach(n => {
          ...
          // Not hovering: stop on back, signal done
    ...
    // Modal content staggers
    tl.to(modalEls, { y: 0, opacity: 1, duration: 0.4, stagger: 0.08 }, '-=0.2');

    // Hold on back — this is where the cycle ENDS
    tl.to({}, { duration: 2 });
  ...
  useEffect(() => {
    ...
    if (isActive) {
      // Reset front elements (hidden behind backface)
      NODES.forEach(n => {
        const el = scene.querySelector(`#${n.id}`);
        if (el) gsap.set(el, { opacity: 0 });
      });
      ...
      // If we're on the back (180°), flip to front first, then restart
  ...
  const scale = isExpanded ? 1 : isHovered ? 0.9 : 0.28;
        ...
        >
            ...
            {NODES.map(n => {
              ...
              return (
                <div key={n.id} id={n.id} style={{
                  ...
                  {/* Handles */}
                  <div style={{
                    position: 'absolute', left: -7, top: n.handleY, transform: 'translateY(-50%)',
                    width: 10, height: 10, borderRadius: '50%',
                    background: t.handleBg, border: `2px solid ${t.bg}`, zIndex: 5,
                  }} />
                  {n.id !== 'node5' && (
                    <div style={{
                      position: 'absolute', right: -7, top: n.handleY, transform: 'translateY(-50%)',
                      width: 10, height: 10, borderRadius: '50%',
                      background: t.handleBg, border: `2px solid ${t.bg}`, zIndex: 5,
                    }} />
                  )}
                  ...
                  <div style={{
                    ...
                    backgroundColor: t.headerBg, color: t.headerColor,
                  }}>
                    <span>{n.icon}</span>
                    <span>{n.title}</span>
                    {n.titleExtra && (
                  ...
                  <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <div>
                        ...
                        <div style={{ fontSize: 11, marginTop: 4, color: t.accent,
                          ...(n.id === 'node5' ? { whiteSpace: 'normal' as const, lineHeight: 1.4, marginTop: 6 } : {}),
                        }}>{n.bodySub}</div>
                      </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              borderRadius: 12,
              backgroundColor: '#16161A', border: '1px solid #2A2B36',
              padding: 48,
            }}>
